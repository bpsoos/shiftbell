...
var _ = Describe("Chore template API", func() {
	...
	It("creates a normalized chore template that can be retrieved", func(ctx SpecContext) {
		...
		Expect(homeResult.Home.Links).To(gstruct.MatchAllKeys(gstruct.Keys{
			...
			shiftbellapi.RelationChoreTemplates: gstruct.MatchAllFields(gstruct.Fields{
		...
		Expect(collectionResult.Collection.Links).To(gstruct.MatchAllKeys(gstruct.Keys{
			shiftbellapi.RelationSelf: gstruct.MatchAllFields(gstruct.Fields{
				"Href": Equal("/chore-templates"),
			}),
		}))
		Expect(collectionResult.Collection.Actions).To(gstruct.MatchAllKeys(gstruct.Keys{
			shiftbellapi.ActionCreateChoreTemplate: gstruct.MatchAllFields(gstruct.Fields{
				"Href":        Equal("/chore-templates"),
				"Method":      Equal(http.MethodPost),
				"ContentType": Equal("application/json"),
				"Fields": gstruct.MatchAllElementsWithIndex(gstruct.IndexIdentity, gstruct.Elements{
		...
		createResult, err := client.CreateChoreTemplate(ctx, shiftbellapi.CreateChoreTemplateParams{
			Action:      createAction,
			Name:        "  Laundry  ",
		...
		getResult, err := client.GetChoreTemplate(ctx, shiftbellapi.GetChoreTemplateParams{
	...
	It("browses a scoped active collection through pagination links", func(ctx SpecContext) {
		collection := discoverChoreTemplateCollection(ctx, client)
		scope := uniqueChoreTemplateName("Browse")
		first := createChoreTemplate(ctx, client, collection, scope+" First", "First description")
		second := createChoreTemplate(ctx, client, collection, scope+" Second", "Second description")
		third := createChoreTemplate(ctx, client, collection, scope+" Third", "Third description")
		createChoreTemplate(
			ctx,
			client,
			collection,
		...
		page, err := client.BrowseChoreTemplates(ctx, shiftbellapi.BrowseChoreTemplatesParams{
		...
		Expect(page.Collection.Items).To(gstruct.MatchAllElementsWithIndex(gstruct.IndexIdentity, gstruct.Elements{
		...
		finalPage, err := client.GetChoreTemplates(ctx, page.Collection.Links[shiftbellapi.RelationNext].Href)
		Expect(err).NotTo(HaveOccurred())
		Expect(finalPage.Collection.More).To(BeFalse())
		Expect(finalPage.Collection.Items).To(gstruct.MatchAllElementsWithIndex(gstruct.IndexIdentity, gstruct.Elements{
			"0": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
	...
	It("rejects a case-insensitive duplicate name with a link to the existing template", func(ctx SpecContext) {
		collection := discoverChoreTemplateCollection(ctx, client)
		name := uniqueChoreTemplateName("Duplicate")
		existing := createChoreTemplate(ctx, client, collection, name, "Existing description")
		createAction := collection.Actions[shiftbellapi.ActionCreateChoreTemplate]

		conflict, err := client.CreateChoreTemplate(ctx, shiftbellapi.CreateChoreTemplateParams{
			Action:      createAction,
		...
		Expect(conflict.ErrorResponse.Links).To(gstruct.MatchAllKeys(gstruct.Keys{
			shiftbellapi.RelationExisting: gstruct.MatchAllFields(gstruct.Fields{
				"Href": Equal(existing.ChoreTemplate.Links[shiftbellapi.RelationSelf].Href),
			}),
		}))
	...
	It("edits an active chore template through its advertised action", func(ctx SpecContext) {
		collection := discoverChoreTemplateCollection(ctx, client)
		...
		editResult, err := client.EditChoreTemplate(ctx, shiftbellapi.EditChoreTemplateParams{
	...
	It("rejects editing a chore template to an existing name", func(ctx SpecContext) {
		...
		existing := createChoreTemplate(ctx, client, collection, existingName, "Existing description")
		...
		conflict, err := client.EditChoreTemplate(ctx, shiftbellapi.EditChoreTemplateParams{
			Action:      editAction,
	...
	It("permanently deactivates a chore template through its advertised action", func(ctx SpecContext) {
		collection := discoverChoreTemplateCollection(ctx, client)
		name := uniqueChoreTemplateName("Deactivate")
		created := createChoreTemplate(ctx, client, collection, name, "Deactivate description")
		selfHref := created.ChoreTemplate.Links[shiftbellapi.RelationSelf].Href
		expectActiveChoreTemplateActions(created.Actions, selfHref)

		deactivated, err := client.DeactivateChoreTemplate(ctx, shiftbellapi.DeactivateChoreTemplateParams{
		...
		active, err := client.BrowseChoreTemplates(ctx, shiftbellapi.BrowseChoreTemplatesParams{
			Href:   collection.Links[shiftbellapi.RelationSelf].Href,
			Search: name,
			State:  "active",
			Limit:  20,
		...
		inactive, err := client.BrowseChoreTemplates(ctx, shiftbellapi.BrowseChoreTemplatesParams{
			Href:   collection.Links[shiftbellapi.RelationSelf].Href,
			Search: name,
			State:  "deactivated",
			Limit:  20,
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(inactive.Collection.Items).To(gstruct.MatchAllElementsWithIndex(gstruct.IndexIdentity, gstruct.Elements{
			"0": gstruct.MatchFields(gstruct.IgnoreExtras, gstruct.Fields{
				"Id":   Equal(created.ChoreTemplate.Id),
				"Name": Equal(name),
			}),
	...
	It("returns recovery controls for an invalid create request", func(ctx SpecContext) {
		...
		invalid, err := client.CreateChoreTemplate(ctx, shiftbellapi.CreateChoreTemplateParams{
			Action:      createAction,
			Name:        "   ",
	...
	It("returns collection navigation for a missing chore template", func(ctx SpecContext) {
		collection := discoverChoreTemplateCollection(ctx, client)
		missingHref := strings.TrimRight(collection.Links[shiftbellapi.RelationSelf].Href, "/") + "/999999999"

		missing, err := client.GetChoreTemplate(ctx, shiftbellapi.GetChoreTemplateParams{
